...
jsPsych.plugins['survey-multi-choice'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    html += ".jspsych-survey-multi-choice-question { margin-top: 2em; margin-bottom: 2em; text-align: left; }"+
      ...
      ".jspsych-survey-multi-choice-option { line-height: 0; }"+ //CSS changed
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      for (var j = 0; j < question.options.length; j++) {
        ...
        html += '<input type="radio" name="'+input_name+'" id="'+input_id+'"'+required_attr+'></input>';
        html += '</div>';//CSS
        //html += '</div>';

      }

      html += '</div>'; //CSS REMOVED
    }
    ...
    if (trial.trial_duration !== null) {
      jsPsych.pluginAPI.setTimeout(function() {
          display_element.querySelector('#jspsych-image-button-response').requestSubmit();
